...
Options:
    -b, --build BUILD       Build to use: uma, rocm, vulkan (default: uma)
...
Examples:
    $0 devstral                     # Start Devstral with defaults
...
Environment:
    ...
    BUILD           Default build (uma/rocm/vulkan)
...
main() {
    ...
    # Validate build
    if [[ ! "$BUILD" =~ ^(uma|rocm|vulkan)$ ]]; then
        error "Invalid build: $BUILD"
        echo "Valid builds: uma, rocm, vulkan"
        exit 1
    fi
